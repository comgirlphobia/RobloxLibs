...
function Toggle:Colorpicker(Options)
    ...
    Colorpicker.Colors.HSV[1], Colorpicker.Colors.HSV[2], Colorpicker.Colors.HSV[3] = H,S,V
    ...
    local ColorpickerInline = Utility.AddDrawing("Square", {
        ...
        Thickness = 0, Color = Library.Theme.Inline, Visible = true, Filled = true
    })
    local ColorpickerOutline = Utility.AddDrawing("Square", {
        ...
        Thickness = 0, Color = Library.Theme.LightContrast, Visible = true, Filled = true
    })
    local ColorpickerBase = Utility.AddDrawing("Square", {
        Size = Vector2.new(ColorpickerInline.Size.X - 2, ColorpickerInline.Size.Y - 2),
        Position = Vector2.new(ColorpickerInline.Position.X + 1, ColorpickerInline.Position.Y + 1),
        Thickness = 0, Color = Options.Color, Visible = true, Filled = true
    })
    local ColorpickerGradient = Utility.AddDrawing("Image", {
        Size = Vector2.new(ColorpickerInline.Size.X - 2, ColorpickerInline.Size.Y - 2),
        Position = Vector2.new(ColorpickerInline.Position.X + 1, ColorpickerInline.Position.Y + 1),
        Data = Library.Theme.Gradient, Transparency = 0.5, Visible = true
    })
    ...
    local InternalInline = Utility.AddDrawing("Square", {
        ...
        Size = Vector2.new(225, 250), Thickness = 0, Color = Library.Theme.Inline, Visible = true, Filled = true, ZIndex = 3
    })
    local InternalOutline = Utility.AddDrawing("Square", {
        ...
        Thickness = 0, Color = Library.Theme.LightContrast, Visible = true, Filled = true, ZIndex = 3
    })
    local InternalTopline = Utility.AddDrawing("Square", {
        ...
        Thickness = 0, Color = Library.Theme.Accent[1], Visible = true, Filled = true, ZIndex = 3
    })
    local InternalTitle = Utility.AddDrawing("Text", {
        Text = ToggleTitle.Text, Position = Vector2.new(InternalOutline.Position.X + 8, InternalOutline.Position.Y + 6),
        Center = false, Outline = false, Font = Library.Theme.Font, Size = Library.Theme.TextSize,
        Color = Library.Theme.Text, Visible = true, ZIndex = 3
    })
    ...
    local InternalBaseInline = Utility.AddDrawing("Square", {
        ...
        Size = Vector2.new(192, 192), Thickness = 0, Color = Library.Theme.Inline, Visible = true, Filled = true, ZIndex = 3
    })
    local InternalBase = Utility.AddDrawing("Square", {
        Size = Vector2.new(188, 188),
        Position = Vector2.new(InternalBaseInline.Position.X + 2, InternalBaseInline.Position.Y + 2),
        Thickness = 0, Color = Color3.fromHSV(H,1,1), Visible = true, Filled = true, ZIndex = 3
    })

    -- === Procedural SV overlays (no external textures) ===
    local SVWidth, SVHeight = InternalBase.Size.X, InternalBase.Size.Y
    local SatLines, ValLines = {}, {}

    for x = 0, SVWidth - 1 do
        SatLines[#SatLines+1] = Utility.AddDrawing("Square", {
            Size = Vector2.new(1, SVHeight),
            Position = Vector2.new(InternalBase.Position.X + x, InternalBase.Position.Y),
            Color = Color3.new(1,1,1),
            Transparency = 1 - (x / (SVWidth - 1)), -- white -> transparent across X
            Filled = true, Visible = true, ZIndex = 4
        })
    end
    for y = 0, SVHeight - 1 do
        ValLines[#ValLines+1] = Utility.AddDrawing("Square", {
            Size = Vector2.new(SVWidth, 1),
            Position = Vector2.new(InternalBase.Position.X, InternalBase.Position.Y + y),
            Color = Color3.new(0,0,0),
            Transparency = (y / (SVHeight - 1)), -- transparent -> black across Y
            Filled = true, Visible = true, ZIndex = 5
        })
    end
    ...
    local InternalHueInline = Utility.AddDrawing("Square", {
        ...
        Size = Vector2.new(16, 188), Thickness = 0, Color = Library.Theme.Inline, Visible = true, Filled = true, ZIndex = 3
    })

    -- === Procedural hue gradient ===
    local HueLines = {}
    for y = 0, InternalHueInline.Size.Y - 2 - 1 do
        local frac = y / (InternalHueInline.Size.Y - 2 - 1)
        HueLines[#HueLines+1] = Utility.AddDrawing("Square", {
            Size = Vector2.new(InternalHueInline.Size.X - 2, 1),
            Position = Vector2.new(InternalHueInline.Position.X + 1, InternalHueInline.Position.Y + 1 + y),
            Color = Color3.fromHSV(frac, 1, 1),
            Transparency = 1, Filled = true, Visible = true, ZIndex = 3
        })
    end

    -- Hue thumb
    local InternalOutlineHuePicker = Utility.AddDrawing("Square", {
        Position = Vector2.new(InternalOutline.Position.X + InternalBase.Size.X + 12, InternalHueInline.Position.Y),
        Size = Vector2.new(InternalHueInline.Size.X + 2, 6), Thickness = 2, Color = Library.Theme.Outline, Visible = true, Filled = false, ZIndex = 3
    })
    local InternalHuePicker = Utility.AddDrawing("Square", {
        ...
        Size = Vector2.new(InternalOutlineHuePicker.Size.X - 2, InternalOutlineHuePicker.Size.Y - 2), Thickness = 2,
        Color = Library.Theme.Text, Visible = true, Filled = false, ZIndex = 3
    })

    -- SV cursor
    local Cursor = Utility.AddDrawing("Image", {
        Size = Vector2.new(6, 6), Data = Library.Theme.SaturationCursor, Transparency = 1, Visible = true, ZIndex = 6
    })

    -- Hex and RGB readouts
    local InternalInlineHex = Utility.AddDrawing("Square", {
        Size = Vector2.new(78, 16),
        Position = Vector2.new(InternalOutline.Position.X + 9, InternalOutline.Position.Y + InternalBase.Size.Y + 31),
        Thickness = 0, Color = Library.Theme.Inline, Visible = true, Filled = true, ZIndex = 3
    })
    ...
    local InternalHex = Utility.AddDrawing("Text", {
        Text = ("#%s"):format(tostring(Options.Color:ToHex())),
        Position = Vector2.new(InternalOutlineHex.Position.X + (InternalOutlineHex.Size.X / 2), InternalOutlineHex.Position.Y),
        Center = true, Outline = false, Font = Library.Theme.Font, Size = Library.Theme.TextSize,
        Color = Library.Theme.Text, Visible = true, ZIndex = 3
    })
    local InternalInlineRGB = Utility.AddDrawing("Square", {
        Size = Vector2.new(128, 16),
        Position = Vector2.new(InternalOutlineHex.Position.X + InternalOutlineHex.Size.X + 10, InternalOutlineHex.Position.Y),
        Thickness = 0, Color = Library.Theme.Inline, Visible = true, Filled = true, ZIndex = 3
    })
    ...
    local InternalRGB = Utility.AddDrawing("Text", {
        Text = ("%s, %s, %s"):format(math.floor(Options.Color.R*255), math.floor(Options.Color.G*255), math.floor(Options.Color.B*255)),
    ...
    Utility.AddConnection(Library.Communication.Event, function(Type, Color)
        if Type == "Accent" then InternalTopline.Color = Color
        elseif Type == "Text" then InternalTitle.Color = Color
        elseif Type == "Inline" then
            InternalInline.Color = Color; InternalBaseInline.Color = Color; InternalHueInline.Color = Color; InternalInlineHex.Color = Color
        elseif Type == "Outline" then
            InternalOutline.Color = Color; InternalOutlineHex.Color = Color
        end
    end)
    ...
    function Colorpicker:SetHue(o)
        local Percent = o.Percent or o.Value
        Colorpicker.Colors.HSV[1] = o.Value
        local hsv = Color3.fromHSV(Colorpicker.Colors.HSV[1], Colorpicker.Colors.HSV[2], Colorpicker.Colors.HSV[3])
        InternalOutlineHuePicker.Position = Vector2.new(InternalOutline.Position.X + InternalBase.Size.X + 12,
                                                        InternalHueInline.Position.Y + (InternalHueInline.Size.Y * Percent) - 4)
        ...
        InternalHex.Text = ("#%s"):format(tostring(hsv:ToHex()))
        InternalRGB.Text = ("%d, %d, %d"):format(math.floor(hsv.R*255), math.floor(hsv.G*255), math.floor(hsv.B*255))
        ColorpickerBase.Color = hsv
        if not o.Visual then Library.Flags[Colorpicker.Flag] = hsv; Colorpicker.Callback(hsv) end
    end
    function Colorpicker:RefreshHue()
        local Percent = math.clamp((UserInput:GetMouseLocation().Y - (InternalHueInline.Position.Y)) / (InternalHueInline.Size.Y), 0, 1)
        local Value = math.floor(Percent * Colorpicker.Decimals) / Colorpicker.Decimals
        Value = math.clamp(Value, 0, 1)
        self:SetHue({ Value = Value, Percent = Percent })
    end

    function Colorpicker:SetSaturationX(o)
        local PercentX = o.Percent or o.Value
        Colorpicker.Colors.HSV[2] = o.Value
        local hsv = Color3.fromHSV(Colorpicker.Colors.HSV[1], Colorpicker.Colors.HSV[2], Colorpicker.Colors.HSV[3])
    ...
    function Colorpicker:SetSaturationY(o)
        local PercentY = o.Percent or 1 - o.Value
        Colorpicker.Colors.HSV[3] = o.Value
        local hsv = Color3.fromHSV(Colorpicker.Colors.HSV[1], Colorpicker.Colors.HSV[2], Colorpicker.Colors.HSV[3])
    ...
    function Colorpicker:Drop(State)
        local vis = State
        InternalInline.Visible = vis; InternalOutline.Visible = vis; InternalTitle.Visible = vis
        InternalBaseInline.Visible = vis; InternalBase.Visible = vis
        InternalTopline.Visible = vis; InternalOutlineHuePicker.Visible = vis; InternalHuePicker.Visible = vis
        InternalInlineHex.Visible = vis; InternalOutlineHex.Visible = vis; InternalHex.Visible = vis
        InternalInlineRGB.Visible = vis; InternalOutlineRGB.Visible = vis; InternalRGB.Visible = vis
        InternalInlineRainbow.Visible = vis; InternalOutlineRainbow.Visible = vis; InternalRainbow.Visible = vis
        Cursor.Visible = vis
        for _,d in ipairs(SatLines) do d.Visible = vis end
        for _,d in ipairs(ValLines) do d.Visible = vis end
        for _,d in ipairs(HueLines) do d.Visible = vis end
        Tab.Dropdowns[Side][ToggleTitle.Text] = vis
    end

    -- Input wiring (unchanged from your version)
    Utility.AddConnection(UserInput.InputBegan, function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 then
            if Utility.OnMouse(ColorpickerOutline) then
                Colorpicker.Dropped = not Colorpicker.Dropped
                Colorpicker:Drop(Colorpicker.Dropped)
            elseif Colorpicker.Dropped and Utility.OnMouse(InternalBase) then
                Colorpicker.SaturationDragging = true
            elseif Colorpicker.Dropped and Utility.OnMouse(InternalHueInline) then
    ...
    Utility.AddConnection(UserInput.InputChanged, function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseMovement then
            if Colorpicker.SaturationDragging then
                local m = UserInput:GetMouseLocation()
                local px = math.clamp((m.X - InternalBase.Position.X) / InternalBase.Size.X, 0, 1)
                local py = math.clamp((m.Y - InternalBase.Position.Y) / InternalBase.Size.Y, 0, 1)
                Colorpicker:SetSaturationX({ Value = px, Percent = px, Visual = true })
                Colorpicker:SetSaturationY({ Value = 1 - py, Percent = py })
    ...
    -- Initialize visuals
    Colorpicker:SetHue({ Value = H, Percent = H, Visual = true })
    Colorpicker:SetSaturationX({ Value = S, Percent = S, Visual = true })
    Colorpicker:SetSaturationY({ Value = V, Percent = 1 - V, Visual = true })

    -- book-keeping and layout
    Section.ContentAxis = Section.ContentAxis + ColorpickerOutline.Size.Y + 8
    Tab.SectionAxis = {
        Side == "Left" and Tab.SectionAxis[1] + ColorpickerOutline.Size.Y + 8 or Tab.SectionAxis[1],
        Side == "Right" and Tab.SectionAxis[2] + ColorpickerOutline.Size.Y + 8 or Tab.SectionAxis[2]
    }
    self:UpdateSizeY(Section.ContentAxis + ColorpickerOutline.Size.Y)
